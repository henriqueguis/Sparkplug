...
import string
import sys
sys.path.insert(0, "sparkplug_b")
sys.path.insert(0, "sparkplug_b_pb2")
#print(sys.path)
    ...
    if tokens[0] == "spBv1.0" and tokens[1] == myGroupId and (tokens[2] == "NCMD" or tokens[2] == "DCMD") and tokens[3] == myNodeName:
        ...
        for metric in inboundPayload.metrics:
            if metric.name == "Node Control/Next Server" or metric.alias == AliasMap.Next_Server:
                ...
                addMetric(payload, 'out_Metric2', AliasMap.Device_Metric2, MetricDataType.Int16, newValue)
                ...
                # Create the DDATA payload - use the alias because this isn't the DBIRTH
                payload = sparkplug.getDdataPayload()
                addMetric(payload, 'out_Metric3', AliasMap.Device_Metric3, MetricDataType.Boolean, newValue)
...
def publishNodeBirth():
    ...
    template = initTemplateMetric(payload, "_types_/Custom_Tanque", None, None)
    ...
    addMetric(template, "Nivel", None, MetricDataType.Int32, 0)    # No alias in UDT members
    addMetric(template, "Vazao", None, MetricDataType.Int32, 0)    # No alias in UDT members
    addMetric(template, "Temp", None, MetricDataType.Int32, 0)  # No alias in UDT members
    ...
    addMetric(template, "Teste UDT", None, MetricDataType.Int32, 0)  # No alias in UDT members
    '''
...
def publishDeviceBirth():
    ...
    # Create the UDT definition value which includes two UDT members and a single parameter and add it to the payload
    template = initTemplateMetric(payload, "Tanque-Resfria-1", AliasMap.Custom_Tanque, "Custom_Tanque")
    templateParameter = template.parameters.add()
    templateParameter.name = "Index_1"
    templateParameter.type = ParameterDataType.String
    templateParameter.string_value = "1"
    addMetric(template, "Nivel", None, MetricDataType.Int32, -0)    # No alias in UDT members
    addMetric(template, "Vazao", None, MetricDataType.Int32, -0)    # No alias in UDT members
    ...
    # Create the UDT definition value which includes two UDT members and a single parameter and add it to the payload
    template = initTemplateMetric(payload, "Tanque-Aquece-1", AliasMap.Custom_Tanque, "Custom_Tanque")
    templateParameter = template.parameters.add()
    templateParameter.name = "Index_2"
    templateParameter.type = ParameterDataType.String
    templateParameter.string_value = "2"
    addMetric(template, "Nivel", None, MetricDataType.Int32, 0)    # No alias in UDT members
    addMetric(template, "Vazao", None, MetricDataType.Int32, 0)    # No alias in UDT members
    ...
    types = [DataSetDataType.Int8, DataSetDataType.Int16, DataSetDataType.Int32]
    dataset = initDatasetMetric(payload, "Matriz", AliasMap.Dataset, columns, types)
    row = dataset.rows.add()
    element = row.elements.add();
    element.int_value = 0
    element = row.elements.add();
...
#client.tls_set(r"C:\lab-01\verne_tls\Volumes\certs\all-ca.crt")
...
while True:
    ...
    templateParameter.type = ParameterDataType.String
    templateParameter.string_value = "1"
    addMetric(template, "Nivel", None, MetricDataType.Int32, -count)    # No alias in UDT members
    addMetric(template, "Vazao", None, MetricDataType.Int32, -count+1)    # No alias in UDT members
    addMetric(template, "Temp", None, MetricDataType.Int32, -count+1)  # No alias in UDT members
    ...
    metric = addMetric(payload, 'Metrica_Teste', AliasMap.Device_Metric1, MetricDataType.Boolean, random.choice([True, False]))
    metric.properties.keys.extend(["Unidade"])
    propertyValue = metric.properties.values.add()
    propertyValue.type = ParameterDataType.String
    propertyValue.string_value = 'MT'
    ...
    templateParameter.type = ParameterDataType.String
    templateParameter.string_value = "1"
    addMetric(template, "RPMs", None, MetricDataType.Int32, count)    # No alias in UDT members
    addMetric(template, "AMPs", None, MetricDataType.Int32, count+1)    # No alias in UDT members
    '''
    ...
    row = dataset.rows.add()
    element = row.elements.add();
    element.int_value = count
    element = row.elements.add();
    element.int_value = count+1
    element = row.elements.add();
    element.int_value = count+2
    row = dataset.rows.add()
    element = row.elements.add();
    element.int_value = count+3
    element = row.elements.add();
    element.int_value = count+4
    element = row.elements.add();
    element.int_value = count+5
    if count > 5:
        row = dataset.rows.add()
        element = row.elements.add();
        element.int_value = count + 2
